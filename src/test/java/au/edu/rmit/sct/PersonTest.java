...
public class PersonTest {
    ...
    public void testAddPerson_valid() {
        ...
        try {
            ...
            String expected =
              ...
              + "32|Highland Street|Melbourne|Victoria|Australia|"
              + "15-11-1990";
            assertEquals(expected, content, "persons.txt should contain exactly one pipe-delimited record");
        } catch (IOException e) {
    ...
    public void testAddPerson_invalidID_length() {
        ...
        assertFalse(p.addPerson(), "An ID shorter than 10 characters should cause addPerson() to return false");

        assertFalse(Files.exists(Paths.get("persons.txt")), "persons.txt should not be created when addPerson fails");
    }
    ...
    public void testAddPerson_invalidAddress_state() {
        ...
        assertFalse(p.addPerson(), "Address whose State is not 'Victoria' should cause addPerson() to return false");
        assertFalse(Files.exists(Paths.get("persons.txt")), "persons.txt should remain absent when address validation fails");
    }
    ...
    public void testAddPerson_invalidBirthdate_format() {
        ...
        assertFalse(Files.exists(Paths.get("persons.txt")), "persons.txt should not be created when birthdate is invalid");
    }
    ...
    public void testAddPerson_invalidID_noSpecials() {
        ...
        assertFalse(p.addPerson(), "An ID with fewer than two special chars in positions 3–8 should fail addPerson()");
        assertFalse(Files.exists(Paths.get("persons.txt")), "persons.txt should not exist after a failed addPerson()");
    }
    ...
    public void testUpdatePersonalDetails_renameOnly() {
        ...
        assertTrue(original.addPerson(), "We must have a valid person in persons.txt for update tests");
        ...
        assertTrue(result,  "Updating only firstName (everything else identical) should succeed");
    }
    ...
    public void testUpdatePersonalDetails_ageUnder18_addressChange() {
        ...
        assertFalse(result, "Under-18 must not be allowed to change address");
    }
    ...
    public void testUpdatePersonalDetails_birthdateChangeOnly() {
        ...
        assertTrue(teen.addPerson(), "Teen must exist before we can update birthdate");
        ...
        assertTrue(result, "Under-18 is allowed to change birthdate only if no other fields change");
    }
    ...
    public void testUpdatePersonalDetails_evenIDchange() {
        ...
        assertTrue(p2.addPerson(), "Person with even-first-digit ID must be created first");
        ...
        assertFalse(result, "Cannot change ID if the first digit is even");
    }
    ...
    public void testUpdatePersonalDetails_invalidNewAddressFormat() {
        ...
        assertFalse(result, "New address missing '|' separators should fail validation");
    }
    ...
    public void testAddDemeritPoints_ptsTooLow() {
        ...
        assertTrue(p.addPerson(), "Person must be added before adding demerit points");

        assertEquals("Failed", p.addDemeritPoints("10-10-2024", 0), "Points <1 should cause addDemeritPoints() to return 'Failed'");
    }
    ...
    public void testAddDemeritPoints_invalidDate() {
        ...
        assertEquals("Failed", p.addDemeritPoints("2024/10/10", 3), "Invalid date format should cause addDemeritPoints() to return 'Failed'");
    }
    ...
    public void testAddDemeritPoints_underThreshold() {
        ...
        assertFalse(p.isSuspended(), "3 points < 12 threshold for over-21 = not suspended");

        assertEquals("Success", p.addDemeritPoints("01-06-2023", 4));
        assertFalse(p.isSuspended(), "Total 7 < 12 = still not suspended for over-21");
    }
    ...
    public void testAddDemeritPoints_under21_exceed() {
        ...
        assertFalse(teen.isSuspended(), "4 points < 6 threshold for under-21 = not suspended");

        assertEquals("Success", teen.addDemeritPoints("01-06-2023", 3));
        assertTrue(teen.isSuspended(), "7 points > 6 threshold for under-21 = should be suspended");
    }
    ...
    public void testAddDemeritPoints_over21_exceed() {
       Person p = new Person(
        ...
        assertFalse(p.isSuspended(), "6 points < 12 threshold for over-21 = not suspended");

        assertEquals("Success", p.addDemeritPoints("01-06-2023", 6));
        assertFalse(p.isSuspended(), "12 points = 12 threshold for over-21 = still not suspended");

        assertEquals("Success", p.addDemeritPoints("01-12-2023", 1));
        assertTrue(p.isSuspended(), "13 points > 12 threshold for over-21 = should be suspended");
    }
}
